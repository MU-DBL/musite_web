...
const Record = (props) =>{
      ...
      let items;
      if(props.jobStatus != 'unsubmit'){
             items=(<div className = {style.item}>
               <div>{props.value}</div>
             ...
             items=(<div className = {style.item}>
               ...
               <div>{props.seqNumber}</div>
               <div>
                 <span title={props.value} onClick = {props.handleShowunsubmit}>Show</span>
                 <span title={props.value} onClick = {props.handleDownload}>Download</span>
                 <span title={props.value} onClick = {props.handleDelete}>Delete</span>
               </div>
      </div>)}
...
class Profile extends React.Component{
  ...
  handleDownload = e => {
   ...
   handleShowunsubmit = e =>{
       let title = e.target.title;
       swal({
              title:"Unsubmit",
              text: "Job:"+title+" haven't been submitted. Please submit the job and check it later or delete the uploaded files and try again!",
              icon: "info",
              button: "Got it!"//,
              //timer: 3000
              });
  ...
  handleRead = () => {
    ...
    $.ajax({
        type: 'post',
        url: '/loadJob',//只把成功submit的返回，在waitinglist 或者tasklist都可以，把其他的只提交input的job删掉,删掉有风险，如果用户开着history页面，同时没有提交，该文件就被删除了。
        data: {
        ...
        success: data => { //this data is the return from fn_load function is the userFiles and it is a folder
          if(data){
            ...
            {
                if(data["jobStatus"][js] != 'completed' && data["jobStatus"][js] !='unsubmit') //unsubmit 不算不complete 不需要改为false
                {
                    allcomplete=false;
                }
            }
...
//如果加了下面这个，几乎会一直update，只有一个好处就是在show 时，如果一个job结束了，running会立即变为complete，如果下面的componentDidUpdate，每隔5秒会检查一下，也很好。
    	      	 ...
    	      	 {records.map((e, idx) =>{
    	      		return (
    	      			<div className = {style.record} key = {e}>
    		      			<Record value = {e} jobStatus = {this.state.jobStatusList[idx]} seqNumber = {this.state.jobSeqnumList[idx]} handleShowResult = {this.props.handleShowResult} handleDownload = {this.handleDownload} handleDelete = {this.handleDelete} handleShowunsubmit={this.handleShowunsubmit}/>
    		      		</div>
    	      		)
    	      	  })}
            </div>
