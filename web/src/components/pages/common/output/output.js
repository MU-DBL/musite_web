...
import { ptmColorConfigs } from '../../../../constants';
...
class Output extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    // Your existing sequence update logic
    let current = this.state.current - 1;
    let title = this.props.title || [];
    let seq = this.props.input || [];
    ...
    // Your existing blast reset logic
    ...
    // Also reset blast when switching sequences
  ...
  render() {
    ...
    if (this.props.isLoading) {
      items = (
        <div className={style.loading}>
          <div className={style.curvalue}>
            {this.props.currentresultstatus}%
          </div>
    ...
    else if (this.props.title.length > 0) {
      items = (
    ...
    return (
      <div className={style.container}>
        ...
        <div className={style.right}>
          <ul>
            {ptmColorConfigs.map((config) => (
              <li key={config.abbreviation}>
                {config.label}:{' '}
